...
> 文档版本：**v1.4**  
> 适用范围：本后端 ↔ 前端  
> 关联文档：[会话标题](会话标题.md)、[联调指南](联调指南.md)
...
| P0 | 资源块渲染 | ✅ | `video` / `link` / `image`（v1.1，兼容保留） |
| P0 | 选集卡片 | ✅ **v1.4 新增** | `resource_bundle`：按线路分组的折叠选集面板 |
...
用户消息 + attachments
    ...
    → SSE 推送 resource_bundle + video/link/image + 短 text_delta 推荐语
    → done
```
...
|-----------|-----------|------|----------|
| `text_delta` | `chunk` | v1.0 | 拼接到助手正文（**v1.4 起不再包含裸播放 URL**） |
| `video` | `chunk` | v1.1 | 推入 `videos[]`；若本轮已有 `resource_bundle`，可忽略以免重复卡片 |
| `link` | `chunk` | v1.1 | 推入 `links[]`，渲染链接列表 |
| `image` | `chunk` | v1.1 | 推入 `images[]`，渲染缩略图（**爬虫结果图，非用户上传图**） |
| `resource_bundle` | `chunk` | **v1.4** | 按线路分组的选集数据，渲染 Tab + 集数方块 |
...
>
> **v1.4**：大模型只输出 2～3 句观影推荐。真实播放地址走 `resource_bundle`（及兼容用的 `video` 块），禁止把签名直链打进打字机文本。
...
#### `resource_bundle`（v1.4，推荐主渲染）

同一作品的多条播放线路（CDN / 插件 road）打包下发。前端应渲染：

- 顶部线路 Tab（`recommended=true` 的标为推荐）
- 主体选集小方块（`【01】` / `第01集`）
- 点击集数：用 DPlayer / ArtPlayer / hls.js 打开对应 `url`（`.m3u8` 或 `.mp4`）
- 可选：复制链接
  ...
  "sources": [
    {
      ...
      "recommended": true,
      "episodes": [
        {"title": "第01集", "url": "https://cdn.example.com/ep1.mp4", "format": "mp4"}
      ]
    },
    {
      ...
      "recommended": false,
      "episodes": [
        {"title": "第01集", "url": "https://cdn.example.com/ep1/index.m3u8", "format": "m3u8"}
      ]
    }
  ]
}
...
同一轮里仍会下发扁平 `video` 块，便于旧前端。**若已处理 `resource_bundle`，不要再为每条 `video` 画一张卡片。**
...
sequenceDiagram
    ...
    loop SSE
        BE-->>FE: resource_bundle / video / link / image
        BE-->>FE: text_delta（短推荐语，无裸 URL）
    end
    BE-->>FE: done
    FE->>FE: 渲染文字 + 资源卡片
```
...
- [ ] 分支处理 `resource_bundle` / `video` / `link` / `image`
- [ ] 处理 `done` / `error`
...
- [ ] 助手消息：分区渲染 `content` / 选集卡片(`resource_bundle`) / `links` / `images`
- [ ] 流式中禁用发送按钮；超时友好提示
...
| 版本 | 说明 |
|------|------|
| **v1.4** | 新增 `resource_bundle` 按线路分组的选集卡片；LLM 不再在 `text_delta` 中倾倒播放 URL |
| v1.2 | 新增图片上传接口、`attachments` 字段、多模态对话流程说明 |
| v1.1 | 启用 video/link/image SSE 资源块 |
| v1.0 | 初版对话 + text_delta 流式协议 |
